...
public class HomeController {
    ...
    @Operation(summary = "查询首页商品推荐列表", description = "查询首页商品推荐列表")
    @GetMapping("/recommend")
    public Result<?> queryRecommendList() {
        LambdaQueryWrapper<BusProduct> lqw = new LambdaQueryWrapper<>();
        lqw.eq(BusProduct::getRecommend, true);
        List<BusProduct> list = productService.list(lqw);

        if (list.isEmpty()) {
            return Result.OK();
        }

        List<HomeProductVo> homeProductVos = productService.buildHomeRecommendList(list);
        return Result.OK(homeProductVos);
    }

    @Operation(summary = "查询首页商品折扣推荐列表", description = "查询首页商品折扣推荐列表")
    @GetMapping("/discount")
    public Result<?> queryDiscountList() {
        LambdaQueryWrapper<BusProduct> lqw = new LambdaQueryWrapper<>();
        lqw.eq(BusProduct::getDiscount, true);
        List<BusProduct> list = productService.list(lqw);
